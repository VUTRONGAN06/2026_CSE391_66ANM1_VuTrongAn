...
> - Điều này cho em biết JavaScript đang làm gì khi trang web được tải?  JavaScript được thực thi khi trang web được tải và có thể hiển thị thông tin hoặc chạy các lệnh trong console.
...
> a) JavaScript chạy ở đâu? (Trình duyệt / Server / Cả hai?)  JavaScript có thể chạy trong trình duyệt web và trên server (ví dụ Node.js).
> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
>
> - HTML: tạo cấu trúc nội dung của trang web.
> - CSS: định dạng giao diện và kiểu hiển thị của trang web.
> - JavaScript: xử lý logic, tương tác và hành vi của trang web.
...
>   - `number` là: kiểu dữ liệu dùng để biểu diễn số.
>   - `string` là: kiểu dữ liệu dùng để biểu diễn chuỗi ký tự (văn bản).
...
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào? JavaScript có thể báo Syntax Error trong console hoặc chương trình không chạy đúng.

---

#### Bước 5: Phản tư nhanh (Reflection)

> - Điều thú vị nhất em vừa khám phá được về console là gì? Console cho phép chạy và kiểm tra các đoạn JavaScript nhanh chóng để xem kết quả ngay lập tức.
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)? Khi gặp lỗi cú pháp, em đọc thông báo lỗi trong console và sửa lại code cho đúng.
...
> a) Nếu `score = 9`, em dự đoán console sẽ in: Console sẽ in Giỏi
> b) Nếu `score = 6`, em dự đoán console sẽ in: Console sẽ in Trung bình
Sau đó:
...
Cấu trúc if/else giống như các ngã rẽ quyết định.
Nếu điều kiện đúng thì chương trình thực hiện lệnh trong if, nếu sai thì chuyển sang else hoặc else if.
...
>   - Thân hàm (body): các câu lệnh bên trong {}
>   - Giá trị trả về (return): kết quả được trả về bằng return
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
Giúp tái sử dụng code, làm chương trình gọn gàng và dễ quản lý hơn.
---
...
> - Phần nào trong if/else hoặc hàm khiến em khó hiểu nhất? Cách hoạt động của if/else khi có nhiều điều kiện.
> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …)? Chạy thử nhiều giá trị khác nhau và quan sát kết quả trong console.
...
> Câu hỏi:
> - `document.getElementById("status")` đang làm gì? Lấy phần tử HTML có id là "status" từ trang web.
> - Sự kiện `"click"` xảy ra khi nào? Khi người dùng nhấn chuột vào phần tử.
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang? Nội dung của đoạn <p> có id "status" được thay đổi bằng JavaScript.
...
> Câu hỏi:
> - Em có thể đổi sang màu khác (vd. `lightblue`) không? Hãy thử. Có. Ví dụ: lightblue, green, yellow.
> - Em hãy ghi lại 1 ví dụ khác mà JavaScript có thể làm với `document.body.style`. Thay đổi kích thước chữ:
...
> Câu hỏi:
> - Sự kiện `"input"` khác gì so với `"click"`? "input" xảy ra khi người dùng gõ hoặc thay đổi nội dung trong ô input, còn "click" xảy ra khi nhấn chuột.
> - Khi em xoá hết nội dung ô input, dòng `greeting` hiển thị gì? Dòng greeting hiển thị:
...
>   DOM là mô hình biểu diễn trang HTML dưới dạng một cây các đối tượng mà JavaScript có thể truy cập và thay đổi nội dung hoặc kiểu hiển thị.
> - Nêu 1 ví dụ “thao tác DOM” trong bài (ghi lại 1 dòng lệnh cụ thể). document.getElementById("status")
...
> Câu hỏi:
> - Điểm giống nhau về chức năng giữa 2 đoạn code trên là gì? Cả hai đoạn code đều xử lý sự kiện click của nút btnHello.
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
JavaScript thuần dùng document.getElementById và addEventListener.
jQuery dùng cú pháp ngắn gọn hơn: $("#id") và .on().
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. Thao tác DOM và xử lý sự kiện dễ dàng hơn.
>   2. Viết JavaScript ngắn gọn và tương thích nhiều trình duyệt.
